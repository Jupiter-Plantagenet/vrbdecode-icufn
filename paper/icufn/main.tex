...
We implement the receipt generator and verifier in Python~3.12 with SHA-256 (Table~\ref{tab:params}). Scaling experiments use Zipf-like logit distributions; we additionally validate on GPT-2 (117M) logits across 100 prompts (4 categories, 3{,}200 steps) at $K\!=\!32$, $N\!=\!32$. All latency measurements are single-threaded. Production deployment would replace SHA-256 with Poseidon for SNARK compatibility~\cite{Nova2022}.
...
Logit distribution    & Zipf-like ($+2.0$ to $-3.0$ Q16.16) \\
Top-$k$               & $\lfloor K/4 \rfloor$ \\
Top-$p$ (Q16.16)      & 0.9 \\
Temperature (Q16.16)  & 1.0 \\
Latency runs per config & 100 (5 warmup discarded) \\
...
\begin{table}[t]
\caption{Verification latency and evidence size ($K$: candidate-set size, $N$: transcript length). Measured on an Intel Core i7 laptop, Python~3.12, Ubuntu~24.04 (WSL~2), 95 measured runs per configuration after 5 warmup.}\label{tab:latency}
\centering
\setlength{\tabcolsep}{3.0pt}
